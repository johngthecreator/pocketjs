// DevTools shim contract tests (DEVTOOLS.md) — drive the runtime shim
// (src/devtools.ts) through the public render() with a mock HostOps + an
// in-process transport, exactly the way a device host injects one. The
// protocol IS the product here: everything the panel and scripts/tape.ts
// rely on is asserted at this layer.
//
// Run: bun test --conditions=browser test/devtools.test.ts
...
import { Named, View } from "../src/components.ts";
import { BTN, ROOT_ID } from "../spec/spec.ts";
